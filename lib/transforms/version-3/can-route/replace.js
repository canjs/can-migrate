...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-route-replace:' + file.path);
  var config = (0, _getConfig2.default)(options.config);
  var j = api.jscodeshift;
  var printOptions = options.printOptions || {};
  var root = j(file.source);
  var found = false;
  var newName = config.moduleToName['can-route'];

  debug('Finding all instances of \'can.route\'');
  root.find(j.MemberExpression).filter(function (expression) {
    var match = true;

    match = match && expression.value.object.name === 'can';

    return match && expression.value.property.name === 'route';
  }).forEach(function (expression) {
    debug('Replacing all instances of \'can.route\' with \'' + newName + '\'');
    found = true;
  ...
  if (found) {
    _dependencyUtils2.default.add(root, 'can-route', newName, ['can', 'can/', 'can/can', 'can/can.js']);
  }
