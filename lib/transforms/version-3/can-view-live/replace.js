...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-view-live-replace:' + file.path);
  var config = (0, _getConfig2.default)(options.config);
  var j = api.jscodeshift;
  var printOptions = options.printOptions || {};
  var root = j(file.source);
  var found = false;
  var newName = config.moduleToName['can-view-live'];

  debug('Finding all instances of \'can.view.live\'');
  root.find(j.MemberExpression).filter(function (expression) {
    ...
    return match && expression.value.property.name === 'live';
  }).forEach(function (expression) {
    debug('Replacing all instances of \'can.view.live\' with \'' + newName + '\'');
    found = true;
  ...
  if (found) {
    _dependencyUtils2.default.add(root, 'can-view-live', newName, ['can', 'can/', 'can/can', 'can/can.js']);
  }
