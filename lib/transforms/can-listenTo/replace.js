...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-listenTo-replace:' + file.path);
  ...
  debug('Finding all instances of \'can.listenTo\'');
  root.find(j.MemberExpression).filter(function (expression) {
    var match = true;

    match = match && expression.value.object.name === 'can';

    return match && expression.value.property.name === 'listenTo';
  }).forEach(function (expression) {
    debug('Replacing all instances of \'can.listenTo\' with \'' + newName + '\'');
    found = true;
