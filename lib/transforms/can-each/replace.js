...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-each-replace:' + file.path);
  var config = (0, _getConfig2.default)(options.config);
  var j = api.jscodeshift;
  var printOptions = options.printOptions || {};
  var root = j(file.source);
  var found = false;
  var newName = config.moduleToName['can-util/js/each/each'];
  ...
  if (found) {
    _dependencyUtils2.default.add(root, 'can-util/js/each/each', newName, ['can', 'can/', 'can/can', 'can/can.js']);
  }
