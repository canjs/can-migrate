...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-stopListening-replace:' + file.path);
  ...
  debug('Finding all instances of \'can.stopListening\'');
  root.find(j.MemberExpression).filter(function (expression) {
    var match = true;

    match = match && expression.value.object.name === 'can';

    return match && expression.value.property.name === 'stopListening';
  }).forEach(function (expression) {
    debug('Replacing all instances of \'can.stopListening\' with \'' + newName + '\'');
    found = true;
