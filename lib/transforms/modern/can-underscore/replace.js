...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-underscore-replace:' + file.path);
  ...
  debug('Finding all instances of \'can.underscore\'');
  root.find(j.MemberExpression).filter(function (expression) {
    var match = true;

    match = match && expression.value.object.name === 'can';

    return match && expression.value.property.name === 'underscore';
  }).forEach(function (expression) {
    debug('Replacing all instances of \'can.underscore\' with \'' + newName + '.underscore\'');
    found = true;

    // can.event.addEvent -> canEvent.addEventListener
    j(expression).replaceWith(j.memberExpression(j.identifier(newName), j.identifier('underscore')));
  });

  if (found) {
    _dependencyUtils2.default.add(root, 'can-util/js/string/string', newName, ['can', 'can/can', 'can/can.js']);
  }
