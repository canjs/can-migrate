...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-one-replace:' + file.path);
  ...
  debug('Finding all instances of \'can.one\'');
  root.find(j.MemberExpression).filter(function (expression) {
    var match = true;

    match = match && expression.value.object.name === 'can';

    return match && expression.value.property.name === 'one';
  }).forEach(function (expression) {
    debug('Replacing all instances of \'can.one\' with \'' + newName + '\'');
    found = true;
