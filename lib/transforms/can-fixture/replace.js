...
function transformer(file, api, options) {
  var debug = (0, _debug2.default)('can-migrate:can-fixture-replace:' + file.path);
  var config = (0, _getConfig2.default)(options.config);
  var j = api.jscodeshift;
  var printOptions = options.printOptions || {};
  var root = j(file.source);
  var found = false;
  var newName = config.moduleToName['can-fixture'];

  debug('Finding all instances of \'can.fixture\'');
  root.find(j.MemberExpression).filter(function (expression) {
    var match = true;

    match = match && expression.value.object.name === 'can';

    return match && expression.value.property.name === 'fixture';
  }).forEach(function (expression) {
    debug('Replacing all instances of \'can.fixture\' with \'' + newName + '\'');
    found = true;
  ...
  if (found) {
    _dependencyUtils2.default.add(root, 'can-fixture', newName, ['can', 'can/', 'can/can', 'can/can.js']);
  }
